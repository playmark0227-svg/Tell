...
商材を入れて GitHub Actions を回すと、**ウェブ検索 → 企業HP特定 → 電話・問い合わせURL抽出**まで自動でやります。

### 検索プロバイダ：Brave Search 推奨（無料2000クエリ/月、全ウェブOK）

| プロバイダ | 無料枠 | 全ウェブ検索 | 必要な準備 |
|---|---|---|---|
| **Brave Search**（推奨） | 月2,000クエリ | ✓ | アカウント＋APIキー |
| Google CSE | 日100クエリ | ✗（2024年廃止） | 利用不可・特定サイトのみ検索 |

商材5個 × 5クエリ = 25クエリ/回。週1実行で月100クエリ → **完全無料**。

### Brave Search セットアップ（5分）

1. https://api.search.brave.com/ にアクセスしてアカウント作成
2. ダッシュボードで「Free」プラン（2000クエリ/月）を選択（クレカ不要）
3. API Keysから新規キーを発行 → コピー（**チャット等に貼らないで**）
4. GitHub → リポジトリ → Settings → Secrets and variables → Actions
5. 「New repository secret」で `BRAVE_API_KEY` を登録
6. 完了
